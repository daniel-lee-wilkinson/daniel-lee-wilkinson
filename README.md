...
> *Each pipeline is built for a different system, but each iteration deliberately improves on the last in architecture, tooling, and reliability.*
...
| [**Industrial Sensor Data Pipeline (Dagster)**](https://github.com/daniel-lee-wilkinson/co2-pipeline-dagster) | Third-generation rebuild with a modern stack. Asset-based orchestration with explicit dependency chains, pre-flight checks (env vars, API auth, UUID reachability), parallelised fetch via `ThreadPoolExecutor`, asset-level data quality checks, and performance monitoring (timing + memory) per stage. All failures raise explicitly and are collected before reporting. 242-test pytest suite with synthetic data generation. CI via GitHub Actions. | ![Language: Python](https://img.shields.io/badge/Language-Python%203.12-blue?logo=python) ![Orchestration: Dagster](https://img.shields.io/badge/Orchestration-Dagster-purple?logo=dagster) ![Library: Polars](https://img.shields.io/badge/Library-Polars-blue?logo=polars) ![Database: DuckDB](https://img.shields.io/badge/Database-DuckDB-yellow?logo=duckdb) ![Storage: Parquet](https://img.shields.io/badge/Storage-Parquet-lightgrey?logo=apacheparquet) ![Tests: pytest (242)](https://img.shields.io/badge/Tests-pytest%20(242)-brightgreen) ![CI: GitHub Actions](https://img.shields.io/badge/CI-GitHub_Actions-brightgreen?logo=github-actions) ![Data Quality: asset checks](https://img.shields.io/badge/Data_Quality-asset--checks-blueviolet) |
...
| Project | Key Tasks | Skills Used |
|---------|-----------|-------------|
| [**Energy Mix Optimisation for Carbon Capture Operations**](https://github.com/daniel-lee-wilkinson/energy_mix_optimisation) *(in progress)* | Hourly optimisation model sizing PV/Wind under land constraints and battery dispatch while minimising lifecycle global warming potential (GWP) of power supplied to carbon capture operations. Includes demand sensitivity analysis, net CO₂ accounting (removal credit), and executive-ready outputs. Pending colleague review before conclusions can be drawn. | ![Language: Python](https://img.shields.io/badge/Language-Python%203.12-blue?logo=python) ![Library: pandas](https://img.shields.io/badge/Library-pandas-green?logo=pandas) ![Library: NumPy](https://img.shields.io/badge/Library-NumPy-green?logo=numpy) ![Optimization: SciPy](https://img.shields.io/badge/Optimization-SciPy-purple?logo=scipy) ![Data: NASA POWER API](https://img.shields.io/badge/Data-NASA%20POWER%20API-lightgrey) ![Visualisation: matplotlib](https://img.shields.io/badge/Visualisation-matplotlib-orange?logo=matplotlib) ![Metric: GWP Minimisation](https://img.shields.io/badge/Metric-GWP%20Minimisation-brightgreen) |
